...
public abstract class IntervalStatisticsBase extends IntervalManagerBase {
	...
	public enum StatisticsTimePeriod {
		MINUTES_10(10, "10 minutes."), MINUTES_30(30, "30 minutes."), HOUR_1(60,
				"1 hour."), HOURS_2(120, "2 hours."), HOURS_5(300,
	...
	private boolean intervalIsOlderThanThreshold(DateTime thresholdDateDatabase,
	...
	protected void calculateStatistics() {
		...
		userTest = aggregateDurations(getEditorIntervals(DocumentType.TEST))
				...
				.plus(aggregateDurations(
						getEditorIntervals(DocumentType.FILENAME_TEST)))
				.plus(aggregateDurations(
						getEditorIntervals(DocumentType.PATHNAMME_TEST)));
		userProduction = aggregateDurations(
				getEditorIntervals(DocumentType.PRODUCTION));
		performDataSanitation();
		
		perspectiveDebug = aggregateDurations(
				getPerspectiveIntervals(Perspective.DEBUG));
		perspectiveJava = aggregateDurations(
				getPerspectiveIntervals(Perspective.JAVA));
		perspectiveOther = aggregateDurations(
				getPerspectiveIntervals(Perspective.OTHER));
	...
	protected List<EditorIntervalBase> getEditorIntervals(DocumentType type) {
		...
		for (EditorIntervalBase interval : getIntervals(
				EditorIntervalBase.class)) {
	 ...
	 *         type.
	 */
	protected List<PerspectiveInterval> getPerspectiveIntervals(
			Perspective type) {
		List<PerspectiveInterval> collectedIntervals = new ArrayList<PerspectiveInterval>();
		for (PerspectiveInterval interval : getIntervals(
				PerspectiveInterval.class)) {
	...
	protected Duration aggregateDurations(
			List<? extends IntervalBase> intervals) {
